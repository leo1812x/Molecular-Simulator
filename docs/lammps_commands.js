...
const lammps_classes_1 = require("./lammps_classes");
...
function create_box(id, ...args) {
    if (exports.currentRegion.style == 'block') {
        ...
        const object = new THREE.Mesh(exports.boxForHelper, new THREE.MeshBasicMaterial());
...
//  lattice         sq  0.1
function lattice(style, scale, ...args) {
    exports.lattice_tyle = "none" || "sc" || "bcc" || "fcc" || "hcp" ||
        "diamond" || "sq" || "sq2" || "hex" || "custom" ?
        style : exports.lattice_tyle;
    exports.lattice_scale = scale;
    //? this is not doing anything right now
    exports.currentLattice = new lammps_classes_1.Lattice(style, scale, ...args);
    //? missing args or "values"    
}
exports.lattice = lattice;
function region(id, style, ...args) {
    exports.currentRegion = new lammps_classes_1.Region(id, style, ...args);
}
...
function atom_styles(style, ...args) {
...
function create_atoms(type, style) {
    ...
    if (exports.boxForHelper != undefined && exports.lattice_tyle != null) {
        ...
        const halfHeight = exports.boxForHelper.parameters.height / 2;
        const halfWidth = exports.boxForHelper.parameters.width / 2;
        const halfDepth = exports.boxForHelper.parameters.depth / 2;
        ...
        for (let i = bounds.x[0]; i <= bounds.x[1]; i += exports.lattice_scale) {
            for (let j = bounds.y[0]; j <= bounds.y[1]; j += exports.lattice_scale) {
                for (let k = bounds.z[0]; k <= bounds.z[1]; k += exports.lattice_scale) {
...
exports.set = set;
//? this is not doing anything right now
function velocity(group, style, ...args) {
    if (group == 'all') {
        if (style == 'create') {
            for (let i = 0; i < setUp.AllElements.length; i++) {
                let element = setUp.AllElements[i];
...
function pair_style(style, ...args) {
